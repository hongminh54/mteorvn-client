...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            MeteorVNClient.LOG.error("Failed to compile vertex shader ({}): {}", vertPath, vertError);
        ...
        if (fragError != null) {
            MeteorVNClient.LOG.error("Failed to compile fragment shader ({}): {}", fragPath, fragError);
        ...
        if (programError != null) {
            MeteorVNClient.LOG.error("Failed to link program: {}", programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(MeteorVNClient.identifier("shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
