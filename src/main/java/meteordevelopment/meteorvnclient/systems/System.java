...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(MeteorVNClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(MeteorVNClient.MOD_ID, file.getName());
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    MeteorVNClient.LOG.error("Error loading {}. Possibly corrupted?", this.name);
                    MeteorVNClient.LOG.info("Saved settings backup to '{}'.", backup);
                    e.printStackTrace();
                }
            }
        } catch (IOException e) {
