...
package meteordevelopment.meteorvnclient.systems.waypoints;
...
import meteordevelopment.meteorvnclient.utils.files.StreamUtils;
...
public class Waypoints extends System<Waypoints> implements Iterable<Waypoint> {
    ...
    @Override
    public void init() {
        File iconsFolder = new File(new File(MeteorVNClient.FOLDER, "waypoints"), "icons");
        iconsFolder.mkdirs();
        ...
        for (File file : files) {
            if (file.getName().endsWith(".png")) {
                try {
                    ...
                    MeteorVNClient.LOG.error("Failed to read a waypoint icon", e);
                }
            }
        }
    }
    ...
    public File getFile() {
        ...
        return new File(new File(MeteorVNClient.FOLDER, "waypoints"), Utils.getFileWorldName() + ".nbt");
    }
    ...
    private void copyIcon(File file) {
        String path = "/assets/" + MeteorVNClient.MOD_ID + "/textures/icons/waypoints/" + file.getName();
        InputStream in = Waypoints.class.getResourceAsStream(path);

        if (in == null) {
            MeteorVNClient.LOG.error("Failed to read a resource: {}", path);
            return;
        }
