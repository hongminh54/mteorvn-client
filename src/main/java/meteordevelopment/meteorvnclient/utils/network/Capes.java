...
public class Capes {
    ...
    public static void init() {
        ...
        MeteorExecutor.execute(() -> {
            ...
            Stream<String> lines = Http.get(CAPE_OWNERS_URL)
                .exceptionHandler(e -> MeteorVNClient.LOG.error("Could not load capes: {}",  e.getMessage()))
                .sendLines();
            if (lines != null) {
                lines.forEach(s -> {
        ...
        MeteorVNClient.EVENT_BUS.subscribe(Capes.class);
    }
    ...
    private static class Cape {
        ...
        public Cape(String name) {
            this.identifier = MeteorVNClient.identifier("capes/" + COUNT++);
            this.name = name;
        }
